 ...
 **/
void 
computeMandelbrotSetRow( int W, int H, int maxIter, int num_ligne, int* pixels, int rank, int nbp)
{
...
computeMandelbrotSet( int W, int H, int maxIter, int rank, int nbp )
{
    ...
    if (rank==0) for (int i=1; i<nbp; i++)
        MPI_Send(&pixels, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
    else 
    {
        std::vector<int> pixels;
        MPI_Status status;
        MPI_Recv(&pixels, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    }
    start = std::chrono::system_clock::now();
    // On parcourt les pixels de l'espace image :
    for ( int i = 0; i < H; ++i )
    {
        if (rank*H/nbp <= i && i < (rank+1)*H/nbp)
        {
            computeMandelbrotSetRow(W, H, maxIter, i, pixels.data() + W*(H-i-1), rank, nbp );
        }
    }
